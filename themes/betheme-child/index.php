...
<div id="Content">
	<div class="content_wrapper clearfix firstclass">
...
<div id="Content">
	<div class="content_wrapper clearfix secondclass">
			...
			<?php if (($filters = mfn_opts_get('blog-filters')) && (is_home() || is_category() || is_tag() || is_author())): ?>
				...
				<section class="section section-filters">
					<div class="section_wrapper clearfix">
						...
						<div class="ph-tag-header">
							<div class="ph-tag-header-top">
								<div class="ph-tag-header-top-inner sssssss">
									<h6>BLOG</h6>
									<h1 class="ph-tag-name">
									<?php 
									  $tag_title = single_tag_title('', false); 
									  $tag_title = str_replace('-', ' ', $tag_title); 
									  $tag_title = ucwords($tag_title); 
									  echo 'Topic: ' . $tag_title; 
									?>
									</h1>
										...
										<div class="author_details_image">
											<div class="author_image">
												<?php 
													...
													echo '<div class="author-image">'. $author_avatar ;
													echo '</div>';
												?>
											</div>
										</div>

										<div class="author_details_name_bio">
											<div class="author_details_name">
												<h4 class="ph-ath-name">
													<?php 
													$author = get_user_by('slug', get_query_var('author_name'));	
									...
									<h1 class="ph-cat-name">
									<?php 
									  $cat_title = single_cat_title('', false); 
									  $cat_title = str_replace('-', ' ', $cat_title); 
									  $cat_title = ucwords($cat_title); 
									  echo 'Topic: ' . $cat_title; 
									?>
									</h1>
								</div>
							</div>
							<div class="ph-tag-header-bottom">
								<div class="ph-tag-header-bottom-left">
									
								</div>
								<div class="ph-tag-header-bottom-right">
<div class="ph-blog-categories-filter">

    <h4>Filter By:</h4>

    <select class="blog-list__category-selector">
      ...
      foreach ($tags as $tag) {
          // Check if the tag has posts assigned to it
          $tag_posts = get_posts(array('tag_id' => $tag->term_id));

          if ($tag_posts) {
              $tag_link = esc_url(get_tag_link($tag->term_id));
              $tag_title = esc_html(str_replace('-', ' ', $tag->name));

              echo '<option value="' . $tag_link . '">' . $tag_title . '</option>';
          }
      }
      ?>
...
<script>
  (function(){
    const catSel = document.querySelector('.blog-list__category-selector');
